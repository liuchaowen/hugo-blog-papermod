document.getElementsByTagName("body")[0].classList.add("list"); //修bug
/*
...
var load =
  '<div id="load" onclick="nextFetch()" ><button class="load-btn button-load">加载更多</button></div>';
var loading =
...
function urlsNow(e) {
  ...
  if (e.classList.contains("url-now")) {
    ...
    domUrls.forEach(function (value, index) {
      domUrls[index].classList.remove("url-now");
    });
...
function uniqueFunc(arr) {
  const res = new Map();
  return arr.filter(
    (item) => !res.has(item.creator) && res.set(item.creator, 1)
  );
}
...
function updateHTMl(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    var bbContREG = data[i].content.replace(
      TAG_REG,
    ...
    result +=
      ...
      new Date(data[i].updatedTs * 1000).toLocaleString() +
      '</span></div><div class="bbs-content"><div class="bbs-text">' +
      bbContREG +
      "</div></div></li>";
  } // end for
...
function fetchDB() {
  ...
  var dbA =
    document.querySelectorAll(
      ".bbs-timeline a[href*='douban.com/subject/']:not([rel='noreferrer'])"
    ) || "";
  if (dbA) {
